...
export const TOOLS = [
	...
	inputSchema: {
		...
		properties: {
			...
			importance: {
				...
				minimum: 0,
				maximum: 1,
				description: "先天重要性 0.0~1.0（可选，缺省 0.5）。请保守评分：0.5=普通可复用记忆，0.6~0.7=持续有帮助或明确重要，0.8=关键架构/重要约束，0.9+=核心且错误代价高；临时、局部信息用 0.35~0.4。不要把普通新记忆默认评为 0.7+。检索时重要的记忆在相似度接近时上浮。",
			},
		},
	...
	},
	{
		...
		inputSchema: {
			type: "object" as const,
			properties: {
				topic_name: { type: "string", description: "主题名称，如 '粒子系统'" },
	...
	},
	{
		...
		description:
			...
			"evidence_hint（渐进式披露）：某些结果的片段中段（非摘要区）可能存在与查询相关的证据内容，但因置信度不足未参与排名竞争，故结果带 evidence_hint 字段（含 score/source_range/snippet/view_id）作为低置信提示。若你觉得该片段主题相关、值得深挖，可调 memory_get_fragment 读取原文核实——这是修复『摘要表达不了、证据被阈值挡掉』场景的关键路径，不要因为它在排名里位置靠后就忽略。",
		inputSchema: {
			type: "object" as const,
			properties: {
	...
	inputSchema: {
		...
		properties: {
			...
			confirmed_by: { type: "string", enum: ["user", "agent"], description: "谁促成本次读原文（可选）：'user'=经用户点头确认采用（金标准成功信号）；'agent'=AI 自行判定要读，未经用户确认" },
	...
	},
	{
		...
		inputSchema: { type: "object" as const, properties: { date: { type: "string", description: "日期 YYYY-MM-DD" } }, required: ["date"] },
	},
	{
		name: "memory_get_topic",
		description: "读取指定主题的多日索引，含各阶段摘要和关联片段 ID。",
		inputSchema: { type: "object" as const, properties: { topic_name: { type: "string", description: "主题名称" } }, required: ["topic_name"] },
	},
	{
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				...
				threshold: { type: "number", minimum: 0, maximum: 1, description: "Jaccard 阈值 0~1，默认 0.3（已按中文 Topic 正负样本校准）" },
